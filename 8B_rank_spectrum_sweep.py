# Phase 8B: Sweep across known rank 0, 1, and 2 elliptic curves to validate BSD resonance behavior
...
# Curve set: known symbolic ranks
curve_catalog = {
    "R0_E1": Eq(y**2, x**3 + x),
    "R0_E2": Eq(y**2, x**3 - 36*x),
    "R1_E1": Eq(y**2, x**3 - 4*x + 4),
    "R1_E2": Eq(y**2, x**3 - x),
    "R2_E1": Eq(y**2, x**3 - 7*x + 6),
    "R2_E2": Eq(y**2, x**3 - 10*x + 9)
}

x_vals = [Rational(n, 1) for n in range(-6, 7)]
results = []

for name, curve in curve_catalog.items():
    rational_points = []
    for x_val in x_vals:
...
print(df_rank)
df_rank.to_csv("phase8b_rank_response_log.csv", index=False)
