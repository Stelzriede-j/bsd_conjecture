# Phase 10B: Run resonance field using valid seeds from R3_C1 and R3_C2 (known rank 3 curves)

from sympy.abc import x, y
from sympy import Eq, Rational, solve, lambdify, latex
...
# Curves with working rational points in-grid
rank3_targets = {
    "R3_C1": Eq(y**2, x**3 - 11*x + 14),
    "R3_C2": Eq(y**2, x**3 - 16*x + 16)
}

resonance_log = []

for name, curve in rank3_targets.items():
    # Find rational points and grid positions
    rational_points = []
    for x_val in x_vals:
    ...
    resonance_log.append({
        ...
        "Recovered Curve": curve_str,
        "Triplet Ops": sorted(set(triplets))
    })
import pprint
pprint.pprint(resonance_log)